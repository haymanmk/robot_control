...
`-DROBOT_CONTROL_DOCS_WARN_AS_ERROR=ON` to make an undocumented public symbol fail the
target.
...
| [0007](docs/adr/0007-rt-platform-on-a-cuda-laptop.md) | One laptop with the NVIDIA driver, needed for CUDA inference. PREEMPT_RT with CPU isolation first; Xenomai only as a fallback. Run `hwlatdetect` before drawing any conclusion about kernels. Amended: the fallback also needs an out-of-band CAN driver. |
| [0008](docs/adr/0008-enforce-cyclic-path-rules-mechanically.md) | The cyclic-path rules are enforced by the kernel, not by review: a seccomp allowlist on the loop thread refuses and names any stray syscall, and page faults are counted. The PREEMPT_RT counterpart of EVL's `EVL_T_WOSS`. |
