...
class FileSink {
 public:
  ...
  void start(robot_control::bridge::BridgeServer& server, unsigned poll_interval_milliseconds = 10);
  ...
  std::size_t drain_once(robot_control::bridge::BridgeServer& server);

 private:
  void run(robot_control::bridge::BridgeServer& server, unsigned poll_interval_milliseconds);
  /// The actual drain, used by both entry points. No ownership check.
  std::size_t drain_unchecked(robot_control::bridge::BridgeServer& server);
