...
#include "robot_control/bridge/layout.hpp"
...
class BridgeServer {
 public:
  ...
  bool publish(const robot_control::telemetry::TelemetryRecord& record) noexcept;

  /// Publish the "what is it doing now" snapshot for live viewers.
  void publish_snapshot(const robot_control::telemetry::StateSnapshot& snapshot) noexcept;
  ...
  [[nodiscard]] std::size_t drain(robot_control::telemetry::TelemetryRecord* out, std::size_t max_records) noexcept;
