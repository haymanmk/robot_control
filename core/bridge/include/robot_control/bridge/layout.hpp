...
#include "robot_control/telemetry/record.hpp"
...
struct CommandRecord {
  ...
  float position[robot_control::telemetry::max_joints];       ///< target position per joint, rad
  float velocity[robot_control::telemetry::max_joints];       ///< target velocity per joint, rad/s
  float torque[robot_control::telemetry::max_joints];         ///< feed-forward torque per joint, N m
  float position_gain[robot_control::telemetry::max_joints];  ///< MIT-mode stiffness per joint
  float velocity_gain[robot_control::telemetry::max_joints];  ///< MIT-mode damping per joint
};
...
struct BridgeHeader {
  ...
  alignas(robot_control::realtime::cache_line_bytes) std::atomic<std::uint64_t> client_heartbeat;

  /// Server liveness, so a client can tell "the RT core died" from "the RT core
  /// is idle".
  alignas(robot_control::realtime::cache_line_bytes) std::atomic<std::uint64_t> server_heartbeat;
  ...
  /// never arrived.
  alignas(robot_control::realtime::cache_line_bytes) std::atomic<std::uint64_t> control_token;

  alignas(robot_control::realtime::cache_line_bytes) std::atomic<std::uint32_t> server_state;  ///< ServerState
  ...
  /// inferring it.
  alignas(robot_control::realtime::cache_line_bytes) std::atomic<std::uint64_t> telemetry_dropped;
...
struct BridgeRegion {
  ...
  robot_control::realtime::Seqlock<robot_control::telemetry::StateSnapshot> snapshot;
  /// RT -> drain thread.
  robot_control::realtime::SingleProducerSingleConsumerRing<robot_control::telemetry::TelemetryRecord, telemetry_ring_capacity>
      telemetry;
  /// Controlling client -> RT.
  robot_control::realtime::SingleProducerSingleConsumerRing<CommandRecord, command_ring_capacity> commands;
};

/// Default name; the leading slash is required by shm_open(3).
inline constexpr const char* default_region_name = "/robot_control_bridge";
