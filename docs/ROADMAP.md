...
| **01b — the cyclic guard** (done) | Can the build catch a syscall or a page fault inside the cyclic path, and name it? | seccomp-BPF allowlists; `SIGSYS` and `si_syscall`; `RUSAGE_THREAD` fault counters; why a refused syscall returning `ENOSYS` is the right failure mode; EVL's `EVL_T_WOSS` and `EVL_HMDIAG_SYSDEMOTE` as the dual-kernel counterpart ([ADR-0008](adr/0008-enforce-cyclic-path-rules-mechanically.md)) |
